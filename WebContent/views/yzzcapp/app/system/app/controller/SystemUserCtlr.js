Ext
		.define(
				'System.controller.SystemUserCtlr',
				{
					...
					onDeptTreepanelItemClick : function(is, record, item,
							index, e, eOpts) {
						var me = this;
					...
					onSysUserAddClick : function() {
						var me = this;
						var selection = me.getDeptTreepanel()
					...
					onSysUserEditClick : function() {
						var me = this;
						var selection = me.getUserGridpanel()
								.getSelectionModel().getSelection();
					...
					onSysSysUserShowClick : function() {
						...
						me.getUserImage().setSrc(
								__ctxPath + '/images?filename='
										+ selection[0].get('picture'));
						win.down('form[itemId=UserDetailForm]').loadRecord(
								selection[0]);
						win.show();
					},
					...
					onSysUserRoleClick : function() {
						...
						win.down('displayfield[itemId=SysUserRoleUserid]')
								.setValue(selection[0].get('userid'));
						win.show();
					},
					...
					onSysSubmitRoleBtnClick : function(btn) {
						var me = this;
						var selection = me.getSysUserRoleGrid()
								.getSelectionModel().getSelection();
						if (selection.length != 1) {
							Ext.ux.Toast.msg("提示", "请选择一条信息！");
							return;
					...
					onSetPostParams : function(obj) {
						var me = this;
						// alert( me.getSysUserDetailFormId().getValue());
						obj.setPostParams({
					...
					onSystemUserShowViewClose : function() {
						var me = this;
						var store = me.getUserGridpanel().getStore();
						store.loadPage(1);
					...
					init : function(application) {
						this
								.control({
									...
									"SystemUserShowView " : {
										close : this.onSystemUserShowViewClose
									}
